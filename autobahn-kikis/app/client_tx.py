...
class ClientSession(ApplicationSession):
    ...
    def onJoin(self, details):
        ...
        #def add(a, b):
        #    print('----------------------------')
        #    print("local py add called on {}".format(self._ident))
        #    return [ a + b, self._ident, self._type]

        #yield self.register(add, u'com.example.add', options=RegisterOptions(invoke=u'roundrobin'))
        #print('----------------------------')
        #print('local py procedure registered: com.example.add')
        ...
        while counter < 10:
            ...
            # CALL
            try:
                res = yield self.call(u'com.example.add', x, 23)
                print('----------------------------')
                #print(' x =', x )
                print(' res =', res )
                print('----------------------------')

                #self.log.info("local py add result: {result}",
                #result=res[0])
            ...
            #print('----------------------------')
            #self.log.info("local py published to 'oncounter' with counter {counter}", counter=counter)
            counter += 1
